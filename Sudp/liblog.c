...
static UINT64 g_dlevel = LEVEL_ERROR;
//static UINT64 g_dlevel = LEVEL_INFORM;
//static UINT64 g_dlevel = LEVEL_DEBUG;

//add by andy open log_file
INT32 open_logfile(char *log_path) 
{
    if (NULL == g_logfile)
    {
        ...
        if (NULL == g_logfile)
        {
			PRINTF(LEVEL_WARNING,"Open log_file fail :%s.\n",strerror(errno));
            return R_ERROR;
        }
        else
        {
...
INT32 open_logfile_t(char *log_perfix)
{
    UINT8 *log_file_path = NULL;
    UINT8 *log_path = NULL;
    time_t log_t = 0;
    struct tm log_tm ={0};
    UINT8 *tmp;
    ...
    if (NULL == g_logfile)
    {
        ...
        snprintf(log_file_path, PATH_MAX, "%s/%s-%04d-%02d-%02d.log",log_perfix,LIBLOG_PREFIX,
...
static INT32 log_init()
{
    ...
    if (NULL == g_logfile)
    {
        ...
        //snprintf(log_file_path, PATH_MAX, "%s/"LIBLOG_PREFIX"-%04d-%02d-%02d.log", tmp,
         //   log_tm.tm_year+1900, log_tm.tm_mon + 1, log_tm.tm_mday);
        snprintf(log_file_path, PATH_MAX, "%s/%s-%04d-%02d-%02d.log", tmp,LIBLOG_PREFIX,
           log_tm.tm_year+1900, log_tm.tm_mon + 1, log_tm.tm_mday);
