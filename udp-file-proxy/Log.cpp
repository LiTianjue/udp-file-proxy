 ...
 */
bool Log::Initialise( const std::string& fileName ,LOG_WHERE log_where) { 
	Log& log = Log::get();
	...
	if( !log.m_initialised ) {
		...
		log.log_where = log_where;

#ifdef THREAD
		MUTEX_SETUP(log.lock);
#endif
